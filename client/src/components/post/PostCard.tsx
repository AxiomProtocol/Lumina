        ...
        <CardContent className="p-4 sm:p-6">
          <div className="flex gap-3 sm:gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {post.postType === "music" && (post as any).linkedMusicTrack && (
                <div className="mt-3 rounded-xl border border-border/50 bg-muted/30 p-3 space-y-2" data-testid="music-post-player">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold truncate">{(post as any).linkedMusicTrack.title}</span>
                  </div>
                  {(post as any).linkedMusicTrack.description && (
                    <p className="text-xs text-muted-foreground">{(post as any).linkedMusicTrack.description}</p>
                  )}
                  <audio
                    src={`/objects/${(post as any).linkedMusicTrack.originalObjectKey.replace(/^\/objects\//, "")}`}
                    controls
                    preload="metadata"
