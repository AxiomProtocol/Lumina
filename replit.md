...
- **Core Social Features**: User profiles, content feeds (friends and algorithmic), diverse content posting (text, image, video), commenting, liking, following, and real-time direct messaging. Video posts support custom thumbnails with three options: auto-generation (ffmpeg extracts frame at 2s), manual frame selection (6 preview frames), and custom image upload. **Video Uploads**: Videos are uploaded directly to Replit Object Storage as MP4 files for native browser playback. Large files (>50MB) use chunked uploads with Content-Range headers. Videos play via HTML5 video element with no transcoding required. OG tags support direct MP4 URLs for Facebook/Twitter sharing.
