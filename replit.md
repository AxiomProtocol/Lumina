...
- **Core Social Features**: User profiles, content feeds (friends and algorithmic), diverse content posting (text, image, video), commenting, liking, following, and real-time direct messaging. Video posts support custom thumbnails with three options: auto-generation (ffmpeg extracts frame at 2s), manual frame selection (6 preview frames), and custom image upload. **Video Streaming**: HLS (HTTP Live Streaming) via `server/hlsTranscode.ts` - uploaded videos are automatically transcoded to H.264/AAC HLS format with 6-second segments, stored in GCS, and played via HLS.js on the ForYou page. The `posts.hlsUrl` column stores the manifest path.
